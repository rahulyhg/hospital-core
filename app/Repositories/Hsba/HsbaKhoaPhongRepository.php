...
class HsbaKhoaPhongRepository extends BaseRepositoryV2
{
    ...
    public function getListPhongHanhChinh() {
        ...
        $where = [
            ['hsba_khoa_phong.benh_vien_id', '=', $this->benhVienId],
            ['hsba_khoa_phong.khoa_chuyen_den', '=', $this->khoaId],
            ['hsba_khoa_phong.phong_chuyen_den', '=', $this->phongId],
            ['hsba_khoa_phong.loai_benh_an', '<>', self::BENH_AN_KHONG_KHAM_BENH]
        ];
    ...
    public function getPhongChoByHsbaId($hsbaId, $phongId)
    {
        $where = [
            ['hsba_khoa_phong.loai_benh_an', '<>', self::BENH_AN_KHONG_KHAM_BENH],
            ['hsba.id', '=', $hsbaId],
            ['hsba_khoa_phong.phong_chuyen_den', '=', $phongId]
        ];
        ...
        $query = $this->model
                ...
                ->leftJoin('khoa', 'khoa.id', '=', 'hsba_khoa_phong.khoa_chuyen_den')
                ->leftJoin('phong', 'phong.id', '=', 'hsba_khoa_phong.phong_chuyen_den')
                ->leftJoin('bhyt', 'bhyt.id', '=', 'hsba_khoa_phong.bhyt_id')
                ->leftJoin('vien_phi', 'vien_phi.hsba_id', '=', 'hsba.id')
                ->leftJoin('stt_phong_kham as sttpk', function($join) use ($hsbaId) {
                    $join->on('sttpk.hsba_id', '=', 'hsba_khoa_phong.hsba_id')
